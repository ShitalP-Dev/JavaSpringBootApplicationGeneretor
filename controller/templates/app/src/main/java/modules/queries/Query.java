...
@AllArgsConstructor
@ToString
public class <%= entityName %>Query extends BaseDto {
    ...
    <%_ }else{ _%>
    <%_ if (relationship.relationshipType === "one-to-many") { _%>
      private List<<%= relationship.otherEntityName %>Query> <%= relationship.relationshipName %> ;
    <%_ } _%>
    <%_ if (relationship.relationshipType === "many-to-one") { _%>
      private <%= relationship.otherEntityName %>Query <%= relationship.relationshipName %> ;
    <%_ } _%>
    <%_ if (relationship.relationshipType === "many-to-many") { _%>
      private List<<%= relationship.otherEntityName %>Query> <%= relationship.relationshipName %> ;
    <%_ } _%>
